...
function updateChrome() {
  ...
  if (counterEditActive && st) {
    st.textContent =
      'Drag computer toys into Computer offers (left). When done, tap the main button (Send counter-offer) — then the computer chooses.';
    if (pl) pl.textContent = 'Editing counter-offer — tap the main button when ready';
    ...
    if (phase === 'p1_build' && proposer === 'p1') {
      ...
      pl.textContent = 'Computer proposed — bottom row (+ then main button to send counter-offer).';
  ...
  if (proposeBtn) {
    const showPrimary =
      (phase === 'p1_build' && proposer === 'p1') ||
      (phase === 'p1_review' && proposer === 'p2' && counterEditActive);
    proposeBtn.hidden = !showPrimary;
    if (showPrimary) {
      let label = 'Propose trade';
      if (counterEditActive) label = 'Send counter-offer';
      else if (p1MustSendRevision) label = 'Send revision to computer';
      proposeBtn.textContent = label;
      let canClick = true;
      if (phase === 'p1_build' && proposer === 'p1') {
        canClick = zones.give.length > 0 || zones.take.length > 0;
      }
      proposeBtn.disabled = !canClick;
      proposeBtn.classList.toggle('ft-propose-btn--disabled', !canClick);
    }
  }
  ...
  if (st && !counterEditActive) {
    if (phase === 'ai_think') {
      ...
      if (p1MustSendRevision) {
        st.textContent =
          'Computer asked for more — adjust the table, then tap Send revision (main button).';
      } else {
        st.textContent =
          'Left column = your shelf; right = computer’s shelf. Main button sends your offer; ✕ clears.';
      }
    } else if (phase === 'p1_review') {
      st.textContent =
        '✓ accept · ✕ reject · + to add computer toys, then main button (Send counter-offer) for their turn.';
    } else if (phase === 'p2_build') {
...
function onPrimaryAction() {
  if (counterEditActive) {
    submitP1CounterOffer();
    return;
  }
  if (p1MustSendRevision) {
    sendRevisionToComputer();
    return;
  }
  proposeTrade();
}
          ...
          () => {
            if (els.status) {
              els.status.textContent =
                'Computer wants more — adjust the trade, then tap Send revision (main button).';
            }
            phase = 'p1_build';
...
function init() {
  ...
  wirePads();
  const proposeBtn = document.getElementById('ftPropose');
  if (proposeBtn) proposeBtn.addEventListener('click', onPrimaryAction);
